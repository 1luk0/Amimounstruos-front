...
public class AguaNoDesperdiciaActivity extends BaseActivity {

  private static final String TAG = "AguaNoDesperdiciaActivity";
  private final int CURSO_ID = 22;
  // La actividad actual marca el fin del Nivel 3.
  private final int NIVEL_DESTINO = 3;
  ...
  /**
   * Envía la actualización del progreso al backend (Nivel 3 completado) y navega a la pantalla de niveles.
   */
  private void updateProgresoAndNavigate() {
    ...
    // Crear el objeto Progreso con los datos para actualizar el Nivel 3 como 'completado'
    ...
    call.enqueue(new Callback<Progreso>() {
      ...
      public void onResponse(Call<Progreso> call, Response<Progreso> response) {
        if (response.isSuccessful()) {
          ...
          makeText(AguaNoDesperdiciaActivity.this, "Error al actualizar. Regresando a niveles.", Toast.LENGTH_SHORT).show();
      ...
      public void onFailure(Call<Progreso> call, Throwable t) {
        ...
        makeText(AguaNoDesperdiciaActivity.this, "Error de red. Regresando a niveles.", Toast.LENGTH_SHORT).show();
        // Fallback de red
        Intent intent = new Intent(AguaNoDesperdiciaActivity.this, NivelesmaActivity.class);
        startActivity(intent);
        finish();
